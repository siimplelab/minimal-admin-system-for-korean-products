...
const metrics = [
  { label: "오늘 신규 가입", value: "128", delta: "+12%", trend: [7, 9, 8, 11, 12, 13, 16] },
  { label: "결제 완료", value: "52", delta: "+6%", trend: [5, 6, 6, 7, 8, 9, 9] },
  { label: "대기 처리", value: "17", delta: "-3%", trend: [5, 5, 4, 4, 3, 3, 2] },
  { label: "중지 계정", value: "4", delta: "0%", trend: [1, 2, 1, 2, 1, 1, 1] },
];
...
const statusDistribution = [
  { name: "사용중", value: 72, color: "#16a34a" },
  { name: "대기", value: 18, color: "#d97706" },
  { name: "중지", value: 10, color: "#dc2626" },
];
...
const incidents = [
  { title: "결제 지연 모니터링", owner: "정산운영팀", status: "대기" as const, due: "11:30" },
  { title: "파트너 권한 정책 반영", owner: "파트너운영팀", status: "사용중" as const, due: "14:00" },
  { title: "구버전 API 종료 공지", owner: "서비스기획팀", status: "중지" as const, due: "16:30" },
  { title: "예약 취소 CS 매크로 점검", owner: "고객지원팀", status: "대기" as const, due: "18:00" },
];
...
const latestNotices = [
  ...
  { title: "신규 파트너 센터 권한 정책", category: "정책 안내", updatedAt: "어제 17:10" },
  { title: "구버전 관리자 페이지 종료 안내", category: "중요 공지", updatedAt: "어제 10:40" },
];
    ...
    <div className="space-y-4">
      <section className="card p-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="rounded-md border border-[var(--border)] bg-[var(--surface-muted)] px-2 py-1 text-[11px] font-semibold text-[var(--foreground-muted)]">
            오늘
          </span>
          <span className="rounded-md border border-[var(--border)] bg-white px-2 py-1 text-[11px] text-[var(--foreground-muted)]">
            최근 7일
          </span>
          <span className="rounded-md border border-[var(--border)] bg-white px-2 py-1 text-[11px] text-[var(--foreground-muted)]">
            최근 1개월
              ...
              <div className="mt-2 grid grid-cols-7 gap-1">
                {item.trend.map((point, index) => (
                  <div
                    ...
                    className="rounded-sm bg-[#dbeafe]"
                    style={{ height: `${Math.max(point * 2.1, 8)}px` }}
                  />
                ))}
              </div>
            </article>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.8fr_1fr]">
        <section className="card p-4">
          <SectionHeader title="주간 주문/결제 추이" description="일자별 주문, 결제, 환불 흐름" />
          <ChartSlot mounted={mounted} heightClassName="h-[290px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData} margin={{ left: -20, right: 8 }}>
                ...
                <Line type="monotone" dataKey="주문" stroke="#1d4ed8" strokeWidth={2.2} dot={false} />
                <Line type="monotone" dataKey="결제" stroke="#0f766e" strokeWidth={2.2} dot={false} />
                <Line type="monotone" dataKey="환불" stroke="#dc2626" strokeWidth={1.8} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </ChartSlot>
        </section>
        ...
        <section className="card p-4">
          <SectionHeader title="계정 상태 분포" description="운영 계정 상태 비중" />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="card p-4">
          <SectionHeader title="채널별 주문 구성" description="신규/재구매 스택 바 차트" />
          <ChartSlot mounted={mounted} heightClassName="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={channelData} margin={{ left: -20, right: 10 }}>
                ...
                <Tooltip contentStyle={chartTooltipStyle} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Bar dataKey="신규" stackId="a" fill="#2563eb" radius={[4, 4, 0, 0]} />
                <Bar dataKey="재구매" stackId="a" fill="#06b6d4" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartSlot>
        </section>
        ...
        <section className="card p-4">
          <SectionHeader title="시간대 문의/처리량" description="운영 문의 처리 능력 추이" />
          <ChartSlot mounted={mounted} heightClassName="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={inquiryByHour} margin={{ left: -20, right: 10 }}>
                ...
                <Tooltip contentStyle={chartTooltipStyle} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Area type="monotone" dataKey="문의" stroke="#0284c7" fill="url(#inquiryColor)" strokeWidth={2} />
                <Area type="monotone" dataKey="처리" stroke="#15803d" fill="url(#resolvedColor)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </ChartSlot>
        </section>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_1.2fr_1fr]">
        <section className="card p-4">
          <SectionHeader title="팀별 SLA 달성률" description="운영 영역별 품질 점수" />
          <ChartSlot mounted={mounted} heightClassName="h-[255px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={slaRadarData} outerRadius="72%">
                ...
                <Tooltip contentStyle={chartTooltipStyle} />
                <Radar dataKey="score" stroke="#1e40af" fill="#3b82f6" fillOpacity={0.25} />
              </RadarChart>
            </ResponsiveContainer>
          </ChartSlot>
        </section>
            ...
            {incidents.map((incident) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-[13px] font-medium text-[var(--foreground-strong)]">{incident.title}</p>
                    <p className="mt-1 text-[12px] text-[var(--foreground-muted)]">담당: {incident.owner}</p>
                  </div>
                  <StatusBadge status={incident.status} />
                </div>
                <p className="mt-2 text-[11px] text-[var(--foreground-muted)]">마감 목표: {incident.due}</p>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="card p-4">
          <SectionHeader title="운영 노트" description="체크리스트와 최근 공지" />
          <Tabs defaultValue="todo">
            ...
            <TabsContent value="todo" className="space-y-2 pt-2">
              {checkList.map((item) => (
                <div key={item} className="panel-inset px-3 py-2 text-[13px] text-[var(--foreground)]">
                  {item}
                </div>
              ))}
            </TabsContent>
            ...
            <TabsContent value="notice" className="space-y-2 pt-2">
              {latestNotices.map((notice) => (
                <div key={notice.title} className="rounded-md border border-[var(--border)] px-3 py-2">
                  <p className="text-[13px] font-medium text-[var(--foreground-strong)]">{notice.title}</p>
